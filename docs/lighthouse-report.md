...
追記（同夜の再計測と調査）：
- 計測する Chrome が午前と違っていた（午前は `Chrome/153.0.8010.12`、夜は PC の Chrome `153.0.0.0`）。夜の公開サイトは技詳細 89〜92、表紙・単語集 100。境目にいるので、**同じ Chrome・空いた PC で計り直すまで「90 以上」とは言わない**。
- 技詳細の長い処理（Lighthouse の見積もりで 250〜360 ms と 110〜200 ms）は、技詳細の画面を最初に組み立てる処理そのもの（Svelte のマウント）。CPU プロファイル（4倍遅延）では、最初に位置や幅を読んだ関数に強制レイアウトが約 130 ms 付く。
- 試したが効かなかったもの（元に戻した）：巻物ストリップの幅を ResizeObserver の最初の通知で受け取る、吹き出しの位置を測るのを次の描画の直前に回す。どちらも長い処理の長さが変わらず、強制レイアウトは別の関数に移るだけだった。
- 効きそうな次の手（未実施）：画面下の方（キーフレームの説明・トグルの本文）を最初の描画の後に組み立てて、最初の組み立てを軽くする。
