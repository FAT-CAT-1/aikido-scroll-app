...
## 7. セキュリティ・DADS 対応の後（2026-09-23 夜、D-43〜D-46）

公開サイト（`0c8176d`）で再計測：表紙 99、単語集 99、**技詳細 85〜91（6回）**。アクセシビリティ・ベストプラクティス・SEO は3画面とも 100。

技詳細の低下が今日の変更によるものかを確かめるため、午前に 96 を出した `0c092d8` と `0c8176d` を手元で並べてビルドし、交互に3回ずつ計測した（同じ PC・同じ時間帯）。

| 版 | Performance | TBT | 技詳細チャンクのスクリプト時間 |
|---|---|---|---|
| `0c092d8`（午前 96） | 64・65・67 | 540・510・420 ms | 849・784・680 ms |
| `0c8176d`（今） | 66・66・67 | 480・370・410 ms | 782・631・674 ms |

（手元の vite preview は圧縮・キャッシュが無いので、点数そのものは公開サイトより低く出る）

→ 2つの版に差は無く、**今日の変更による低下ではない**。同じコードでも午前はスクリプト時間が約 300 ms で、この夜は約 2 倍。計測した PC の負荷によるもの（Lighthouse の benchmarkIndex も午前の 4200〜4400 に対し夜は 2700〜4100）。**PC が空いているときに再計測して 90 以上を確かめる**こと（手順は §4）。
